...
type Props = {
  players: Player[];
  draftKnightOwnerId: number | null;
  selectablePlayers: Player[];
  canGoNext: boolean;
  onSelectKnight: (playerId: number) => void;
  onBack: () => void;
  onNext: () => void;
};

export default function FirstNightKnightScreen({
  players: _players,
  draftKnightOwnerId,
  selectablePlayers,
  canGoNext,
  ...
  function speakKnightLine(text: string) {
  ...
  function handleSelectKnight(playerId: number) {
    onSelectKnight(playerId);
    speakKnightLine('骑士上线，白天发言时段你可以随时翻牌，指定一名玩家决斗，每局限用一次。');
  }
    ...
    <section className="bg-[var(--color-wolf-card)] rounded-2xl p-5 mb-5 shadow-[var(--shadow-card)] border border-[var(--color-wolf-border)]">
      <div className="mt-3.5 p-4 rounded-xl bg-[#0e0b1f] border border-[#3730a3]">
        ...
        <div className="text-[var(--color-moon-bright)] font-semibold leading-relaxed">
          <JudgeScriptLines
            lines={[
              { zh: '骑士请睁眼。', en: 'Knight, please open your eyes.' },
            ]}
          />
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="flex flex-wrap gap-2">
          {selectablePlayers.map((player) => {
            const selected = draftKnightOwnerId === player.id;
            return (
              <PlayerSelectButton
                key={player.id}
                player={player}
                selected={selected}
                onClick={() => handleSelectKnight(player.id)}
              />
            );
          })}
        </div>
      ...
      <div className="mt-4 p-4 rounded-xl bg-[#0e0b1f] border border-[#3730a3]">
        <JudgeScriptHeader />
        <div className="text-[var(--color-moon-bright)] font-semibold leading-relaxed">
          <JudgeScriptLines lines={[{ zh: '骑士请闭眼。', en: 'Knight, please close your eyes.' }]} />
        </div>
      </div>
